...
const injectedRtkApi = api
    ...
    endpoints: (build) => ({
        ...
        deleteApiRoleDel: build.mutation<DeleteApiRoleDelApiResponse, DeleteApiRoleDelApiArg>({
            query: (queryArg) => ({
                url: `/api/role/del`,
                method: "DELETE",
        ...
        findAllByDtsIsEmpty2: build.query<FindAllByDtsIsEmpty2ApiResponse, FindAllByDtsIsEmpty2ApiArg>({
            query: () => ({ url: `/api/role` }),
            providesTags: ["Roles"],
        }),
        getLegalStatus1: build.query<GetLegalStatus1ApiResponse, GetLegalStatus1ApiArg>({
...
export type PermissionType = "USER" | "ROLE" | "LOAN" | "LOAN_ROLE" | "LOAN_USER" | "LOAN_FLOW" | "CHANGE_LEGAL_STATUS" | "NOTIFICATION_TEXT" | "REPORT_BY_LEGAL_DOCUMENT" | "CONFIGURATION" | "CHANGE_LEGAL_USER_STATUS";
export type CommonEnumDto = {
    label: string;
    value: string;
};
export const { useUpdate1Mutation, useDeleteApiRoleDelMutation, useAdd1Mutation, useFindAllByDtsIsEmpty2Query, useGetLegalStatus1Query, } = injectedRtkApi;
